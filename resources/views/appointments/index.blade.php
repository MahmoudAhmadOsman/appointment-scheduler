...
=================================== -->
  ...
  <div class="container" style="display: none;">
   <h2 class="text-danger" style="text-align: center; margin-bottom: 40px">Appointment Center</h2>
    <div class="row">
     
       @if(count($appointments) > 0)
 @foreach($appointments as $appointment)
      <div class="col-md-4" style="background-color: #fdfdfd; color: #000;">
        <div class="patient__profile">
          <img src="http://www.writingcenter.uci.edu/wp-content/uploads/sites/28/2017/06/empty-calendar-2400px-copy.png" alt="" style="width: 120px; height: 120px;">
 Scheduled On: {{$appointment->created_at->format('m-d-Y')}}
          <hr>
        </div>
      </div>
...
<div class="col-md-6">
  <h4>Appointment Type: <a href="/appointments/{{$appointment->id}}">{{$appointment->title}}</a></h4>
            <p><b>Patient Full Name:</b> {{$appointment->name}}</p>
            <p><b>Short Details:</b> {{$appointment->details}}</p>  
            <p><b>Appointment Time:</b> {{$appointment->appTime}}</p>
            <p><b>Appointment Location:</b> {{$appointment->location}}</p>
            <p><b>Docotor Name:</b> {{$appointment->doctor}}</p>
            <p><b>Appointment Scheduler Name:</b> {{$appointment->appMaker}}</p><hr>
</div>
<!-- End of the center section -->

<!-- start of the right section -->
<div class="col-md-2" style="background-color: #fdfdfd;">
  <h5>New Appointment?</h5>
  <a href="/appointments/create" class="btn btn-outline-success btn-sm"> Create New Appointment</a>
  </div>
